//
// Created by haoming on 28.03.22.
//
...
namespace fgo::utils {
    ...
    public:
        ...
        void setPPS(const irt_msgs::msg::PPS::ConstSharedPtr msg)
        {
          ...
          //RCLCPP_INFO_STREAM(rosNodePtr_.get_logger(), "Calculate Delay: local delay in s " << local_delay_);
          last_pps_time_ = thisTime;
        }

        void setTOW(double tow)
        {
          //RCLCPP_INFO_STREAM(rosNodePtr_.get_logger(), "TOW: " << std::fixed << tow);
          tow_millisec_ = tow;
          mutex_.lock();
        ...
        [[nodiscard]] double getDelay()
        {
         ...
         // RCLCPP_WARN_STREAM(rosNodePtr_.get_logger(), target_ << " intern delay: " << calculator_output_.MeasurementDelayCalc);
          return delay;
        }
    };
}
